...
async function takeSnapshot(reason = '', kind = 'auto') {
  ...
  // 보관 개수 제한: 최근 30개만 유지 (자동 정리)
  await db.query(`DELETE FROM tb_snapshot WHERE snapshot_id NOT IN
    (SELECT snapshot_id FROM tb_snapshot ORDER BY created_at DESC LIMIT 30)`);
  ...
  try {
    const rows = (await db.query(
      `SELECT snapshot_id,reason,kind,course_cnt,to_char(created_at,'YYYY-MM-DD HH24:MI:SS') AS created_at
       FROM tb_snapshot ORDER BY created_at DESC LIMIT 30`
    )).rows;
...
router.post('/:id/restore', async (req, res, next) => {
  ...
  try {
    ...
    await c.query('BEGIN');
    await c.query('TRUNCATE tb_course_comp,tb_course_map,tb_course_tag,tb_course,tb_competency,tb_job_taxonomy,tb_framework,tb_institution RESTART IDENTITY CASCADE');
